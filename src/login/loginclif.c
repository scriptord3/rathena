 ...
 *  8 : already online
 */
static void login_sent_auth_result(int fd,char result){
	WFIFOHEAD(fd,3);
	WFIFOW(fd,0) = 0x81;
	WFIFOB(fd,2) = result;
	WFIFOSET(fd,3);
...
static void login_auth_ok(struct login_session_data* sd)
{
	...
	if( runflag != LOGINSERVER_ST_RUNNING )
	{
		...
		login_sent_auth_result(fd,1); // server closed
		return;
	}
...

	{
		...
		{// account is already marked as online!
			...
			{// Request char servers to kick this account out. [Skotlex]
				...
				login_sent_auth_result(fd,8); // 08 = Server still recognizes your last login
				return;
			}
			else
			if( data->char_server == -1 )
...
static int login_parse_keepalive(int fd){
	if (RFIFOREST(fd) < 26)
		return 0;
	RFIFOSKIP(fd,26);
	return 1;
...
static int login_parse_updclhash(int fd, struct login_session_data *sd){
	if (RFIFOREST(fd) < 18)
		return 0;
...
static int login_parse_reqauth(int fd, struct login_session_data *sd, int command, char* ip){
		{
...
static int login_parse_reqkey(int fd, struct login_session_data *sd){
	RFIFOSKIP(fd,2);
	{
...
static int login_parse_reqcharconnec(int fd, struct login_session_data *sd, char* ip){
	if (RFIFOREST(fd) < 86)
		return 0;
	{
		int result;
